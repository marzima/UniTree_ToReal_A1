...
public:
    TwistDriver() : Node("a1_twist_driver")
    {
        ...
        this->declare_parameter("using_low_state_publisher", false);//---
        is_walking_ = this->get_parameter("start_walking").as_bool();
        using_imu_publisher = this->get_parameter("using_imu_publisher").as_bool();
        using_low_state_publisher = this->get_parameter("using_low_state_publisher").as_bool();//---
        
        // Initilize publishers
        high_state_pub = this->create_publisher<ros2_unitree_legged_msgs::msg::HighState>("state", 10); //Declare also for dorce here
        if (using_low_state_publischer)
            low_state_pub = this->create_publisher<ros2_unitree_legged_msgs::msg::LowState>("low_state", 10);////----
    ...
    rclcpp::Publisher<ros2_unitree_legged_msgs::msg::LowState>::SharedPtr low_state_pub;//--
    ...
    bool using_low_state_publisher = false;//--
...
int main(int argc, char *argv[])
{
    ...
    while (rclcpp::ok())
    {
        ...
        if (node->using_low_state_publisher)//--
            node->low_state_pub->publish(low_state_ros.low_state);//--
